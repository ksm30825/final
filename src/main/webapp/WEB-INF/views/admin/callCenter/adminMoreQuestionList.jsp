...
<body>
	...
	<div class="columns">
		<div class="column">
			...
			<div class="buttons">
				<a class="button is-success is-rounded" href="insertMoreQuestionForm.ad">작성하기</a>
			</div>
		...
		<nav class="pagination is-rounded" role="navigation"
			...
			<c:if test="${ pi.currentPage > 1 }">
				<c:url var="previous" value="adminMoreQuestionList.ad">
			...
			<c:forEach var="p" begin="${ pi.startPage }" end="${ pi.endPage }">
				...
				<c:if test="${ p ne pi.currentPage }">
					<c:url var="number" value="adminMoreQuestionList.ad">
			...
			<c:if test="${ pi.currentPage < pi.maxPage }">
				<c:url var="next" value="adminMoreQuestionList.ad">
					<c:param name="currentPage" value="${ pi.currentPage + 1 }"/>
				</c:url>
...
<script>
	$(function() {
		...
		$(".table").find("td").mouseenter(function() {
			...
			console.log(boardId);
			location = "adminMoreQuestionDetail.ad?boardId=" + boardId;
		});
	});
</script>
</html>